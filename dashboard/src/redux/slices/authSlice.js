...
export const addNotify = createAsyncThunk('auth/addNotify', async (data, { rejectWithValue }) => {
  try {
    const response = await AuthService.addNotify(data);
    return response;
  } catch (err) {
    return rejectWithValue(err.response.data);
  }
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addNotify.pending, (state) => {
        state.statusAddNotify = 'loading';
      })
      .addCase(addNotify.fulfilled, (state, action) => {
        state.statusAddNotify = 'successful';
        state.auth = action.payload;
      })
