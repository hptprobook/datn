...
import { setStatus, getOneCB, updateCustormerGroup } from 'src/redux/slices/CustormerGroupSlice';
...
export default function DetailCustormerGroupPage() {
  ...
  const status = useSelector((state) => state.CustormerGroups.statusUpdate);
  const error = useSelector((state) => state.CustormerGroups.error);
  const custormerGroup = useSelector((state) => state.CustormerGroups.CustormerGroup);
  const [manual, setManual] = useState(custormerGroup.manual);
  const formik = useFormik({
    initialValues: {
      name: custormerGroup.name || '',
      note: custormerGroup.note || '',
      manual: custormerGroup.manual || manual,
      satisfy: custormerGroup.satisfy || 'all',
      ...(manual === false && {
        auto: [
          {
            field: custormerGroup.field || '',
            query: custormerGroup.query || '',
            status: custormerGroup.status || '',
          }
        ],
      }),
      listCustomer: custormerGroup.listCustomer || [],
    },
    ...
    onSubmit: (values) => {
      ...
      console.log(values);
      dispatch(updateCustormerGroup({ id, data: values }));
    },
  });

  useEffect(() => {
    if (custormerGroup && custormerGroup.manual !== undefined) {
      setManual(custormerGroup.manual);
    }
    if (status === 'failed') {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [custormerGroup, status, error, dispatch]);
    ...
    <Container>
      ...
      <form onSubmit={formik.handleSubmit}>
        <Grid2 container spacing={3}>
          ...
          <Grid2 xs={12}>
            <Card sx={{ padding: 3 }}>
              {custormerGroup.manual && (
                <Box sx={{ mt: 3 }}>
                  <Typography variant="h6" sx={{ mb: 3 }}>
                    Danh sách khách hàng
                  </Typography>
                  {/* Render listCustomer here */}
                </Box>
              )}
                 {!custormerGroup.manual && (
                <Box sx={{ mt: 3 }}>
