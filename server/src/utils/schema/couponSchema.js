...
export const CREATE_COUPONS = Joi.object({
    ...
    status: Joi.string()
        ...
        .messages({
            ...
            'any.only':
                'Trạng thái phải là một trong các giá trị sau: active, inactive, expired.',
        }),
    limitOnUser: Joi.boolean().default(false).messages({
        'boolean.base':
...
export const UPDATE_COUPONS = Joi.object({
    ...
    maxPurchasePrice: Joi.number().min(Joi.ref('minPurchasePrice')).messages({
        'number.base': 'Giá trị mua tối đa phải là một số.',
        'number.min':
            'Giá trị mua tối đa phải lớn hơn hoặc bằng giá trị mua tối thiểu.',
    }),
