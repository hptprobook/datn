...
export const CREATE_STAFF_SCHEMA = Joi.object({
  ...
  staffCode: Joi.string().trim().min(4).max(20).required().messages({
...
export const UPDATE_STAFF_SCHEMA = Joi.object({
  ...
  password: Joi.string().trim().min(1).messages({
    'string.empty': 'Mật khẩu không được để trống',
  }),
  otp: Joi.string(),
  branchId: Joi.string()
  ...
  role: Joi.string().valid('root', 'admin', 'staff', 'ban').messages({
    'any.only': 'Vai trò không hợp lệ',
  }),
  updatedAt: Joi.date().timestamp('javascript').default(Date.now),
}).messages({
...
export const UPDATE_ME_SCHEMA = Joi.object({
  ...
  avatar: Joi.string().max(255).allow(null).messages({
    'string.max': 'Đường dẫn ảnh không được vượt quá 255 ký tự',
    'string.base': 'Đường dẫn ảnh không hợp lệ',
  }),
  password: Joi.string().trim().min(1).messages({
