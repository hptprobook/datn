...
export const SAVE_PAYMENT = Joi.object({
    ...
    status: Joi.string()
        ...
        .messages({
            ...
            'any.only': 'Trạng thái phải là một trong các giá trị sau: pending, completed',
        }),
    ...
    shippingInfo: Joi.object({
            transactionMethod: Joi.string()
                ...
                .messages({
                    ...
                    'string.min': 'Phương thức giao dịch phải có ít nhất {#limit} ký tự',
                    'any.only': 'Phương thức giao dịch phải là một trong các giá trị sau: card, bank, wallet',
                }),
            ...
            status: Joi.string().trim().min(1).valid('failed', 'success').messages({
                ...
                'string.empty': 'Trạng thái không được để trống',
                'string.min': 'Trạng thái phải có ít nhất {#limit} ký tự',
                'any.only': 'Trạng thái phải là một trong các giá trị sau: failed, success',
            }),
            date: Joi.date().timestamp('javascript').default(Date.now),
            cardType: Joi.string().trim().min(1).valid('visa', 'mastercard').messages({
                ...
                'any.only': 'Loại thẻ phải là một trong các giá trị sau: visa, mastercard',
            }),
            ...
            bankholderName: Joi.string().trim().min(1).messages({
                ...
                'string.min': 'Tên chủ thẻ ngân hàng phải có ít nhất {#limit} ký tự',
            }),
